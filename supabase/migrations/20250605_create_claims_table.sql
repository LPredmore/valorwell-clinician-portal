...
CREATE POLICY "Clinicians can view their own claims" ON public.claims
    FOR SELECT USING (
        claims_clinician_id = auth.uid()::text::uuid
    );
...
-- Policy for admins to manage all claims
...
VALUES (
    '20250605_create_claims_table',
    'Created dedicated claims table for billing and claims processing',
    jsonb_build_object(
        'action', 'create_table',
        'table_name', 'claims',
