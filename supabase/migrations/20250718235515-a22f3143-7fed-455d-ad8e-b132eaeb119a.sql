-- Fix the handle_new_user trigger to avoid enum casting issues
...
BEGIN
  ...
  ELSIF v_role = 'clinician' THEN
    -- For clinician role, create clinician record WITHOUT setting clinician_status
    -- Let the column default handle the status value to avoid enum issues
