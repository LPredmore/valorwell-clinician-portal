...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('Starting Hawaii timezone appointment reminder function');
    
    // Calculate 24-48 hour window for Pacific/Honolulu timezone
    const now = new Date();
    const hawaiiNow = new Date(now.toLocaleString("en-US", {timeZone: "Pacific/Honolulu"}));
    
    const start24Hours = new Date(hawaiiNow);
    start24Hours.setDate(start24Hours.getDate() + 1);
    start24Hours.setHours(0, 0, 0, 0);
    
    const end48Hours = new Date(hawaiiNow);
    end48Hours.setDate(end48Hours.getDate() + 2);
    end48Hours.setHours(23, 59, 59, 999);

    console.log(`Searching for appointments between ${start24Hours.toISOString()} and ${end48Hours.toISOString()} for Hawaii timezone`);

    // Query appointments for Hawaii timezone clients in the next 24-48 hours
    const { data: appointments, error } = await supabase
      ...
      .select(`
        ...
        status
      `)
      .eq('client_timezone', 'Pacific/Honolulu')
    ...
    console.log(`Found ${appointments?.length || 0} appointments for Hawaii timezone`);

    if (!appointments || appointments.length === 0) {
      return new Response(JSON.stringify({ message: 'No appointments found for Hawaii timezone' }), {
        status: 200,
    ...
    return new Response(JSON.stringify({
      message: `Processed ${appointments.length} appointments for ${clientGroups.size} clients in Hawaii timezone`,
    ...
    console.error('Error in Hawaii timezone reminder function:', error);
