...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('Starting Eastern timezone appointment reminder function');
    
    // Calculate 24-48 hour window for America/New_York timezone
    const now = new Date();
    const easternNow = new Date(now.toLocaleString("en-US", {timeZone: "America/New_York"}));
    
    const start24Hours = new Date(easternNow);
    start24Hours.setDate(start24Hours.getDate() + 1);
    start24Hours.setHours(0, 0, 0, 0);
    
    const end48Hours = new Date(easternNow);
    end48Hours.setDate(end48Hours.getDate() + 2);
    end48Hours.setHours(23, 59, 59, 999);

    console.log(`Searching for appointments between ${start24Hours.toISOString()} and ${end48Hours.toISOString()} for Eastern timezone`);

    // Query appointments for Eastern timezone clients in the next 24-48 hours
    const { data: appointments, error } = await supabase
      ...
      .select(`
        ...
        status
      `)
      .eq('client_timezone', 'America/New_York')
    ...
    console.log(`Found ${appointments?.length || 0} appointments for Eastern timezone`);

    if (!appointments || appointments.length === 0) {
      return new Response(JSON.stringify({ message: 'No appointments found for Eastern timezone' }), {
        status: 200,
    ...
    return new Response(JSON.stringify({
      message: `Processed ${appointments.length} appointments for ${clientGroups.size} clients in Eastern timezone`,
    ...
    console.error('Error in Eastern timezone reminder function:', error);
