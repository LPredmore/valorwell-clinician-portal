...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('Starting Pacific timezone appointment reminder function');
    
    // Calculate 24-48 hour window for America/Los_Angeles timezone
    const now = new Date();
    const pacificNow = new Date(now.toLocaleString("en-US", {timeZone: "America/Los_Angeles"}));
    
    const start24Hours = new Date(pacificNow);
    start24Hours.setDate(start24Hours.getDate() + 1);
    start24Hours.setHours(0, 0, 0, 0);
    
    const end48Hours = new Date(pacificNow);
    end48Hours.setDate(end48Hours.getDate() + 2);
    end48Hours.setHours(23, 59, 59, 999);

    console.log(`Searching for appointments between ${start24Hours.toISOString()} and ${end48Hours.toISOString()} for Pacific timezone`);

    // Query appointments for Pacific timezone clients in the next 24-48 hours
    const { data: appointments, error } = await supabase
      ...
      .select(`
        ...
        status
      `)
      .eq('client_timezone', 'America/Los_Angeles')
    ...
    console.log(`Found ${appointments?.length || 0} appointments for Pacific timezone`);

    if (!appointments || appointments.length === 0) {
      return new Response(JSON.stringify({ message: 'No appointments found for Pacific timezone' }), {
        status: 200,
    ...
    return new Response(JSON.stringify({
      message: `Processed ${appointments.length} appointments for ${clientGroups.size} clients in Pacific timezone`,
    ...
    console.error('Error in Pacific timezone reminder function:', error);
