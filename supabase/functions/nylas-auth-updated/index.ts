...
// Enhanced JWT verification helper function
async function verifyAuth(req: Request) {
  ...
  // Try to get the user with retry logic for transient errors
  let retries = 2
  let user = null
  let authError = null
  ...
  while (retries >= 0 && !user) {
    try {
      ...
      if (error) {
        console.error(`[nylas-auth] Authentication error (attempt ${2 - retries}/2):`, error)
        authError = error
        retries--
        ...
        retries = -1 // Exit the loop if we got a response but no user
      }
    } catch (error) {
      console.error(`[nylas-auth] Unexpected auth error (attempt ${2 - retries}/2):`, error)
      authError = error
      retries--
  ...
  if (!user) {
    return { 
      ...
      error: {
        code: 'INVALID_JWT',
        message: authError?.message || 'Invalid or expired JWT token'
      }
    }
  }
...
function createErrorResponse(error: { code: string, message: string }, status = 401) {
  return new Response(
    JSON.stringify({ 
...
serve(async (req) => {
  ...
  try {
    ...
    console.error('[nylas-auth] Unhandled error:', error)
    return new Response(
      JSON.stringify({ 
