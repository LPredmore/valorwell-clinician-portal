...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('Starting Alaska timezone appointment reminder function');
    
    // Calculate 24-48 hour window for America/Anchorage timezone
    const now = new Date();
    const alaskaNow = new Date(now.toLocaleString("en-US", {timeZone: "America/Anchorage"}));
    
    const start24Hours = new Date(alaskaNow);
    start24Hours.setDate(start24Hours.getDate() + 1);
    start24Hours.setHours(0, 0, 0, 0);
    
    const end48Hours = new Date(alaskaNow);
    end48Hours.setDate(end48Hours.getDate() + 2);
    end48Hours.setHours(23, 59, 59, 999);

    console.log(`Searching for appointments between ${start24Hours.toISOString()} and ${end48Hours.toISOString()} for Alaska timezone`);

    // Query appointments for Alaska timezone clients in the next 24-48 hours
    const { data: appointments, error } = await supabase
      ...
      .select(`
        ...
        status
      `)
      .eq('client_timezone', 'America/Anchorage')
    ...
    console.log(`Found ${appointments?.length || 0} appointments for Alaska timezone`);

    if (!appointments || appointments.length === 0) {
      return new Response(JSON.stringify({ message: 'No appointments found for Alaska timezone' }), {
        status: 200,
    ...
    return new Response(JSON.stringify({
      message: `Processed ${appointments.length} appointments for ${clientGroups.size} clients in Alaska timezone`,
    ...
    console.error('Error in Alaska timezone reminder function:', error);
