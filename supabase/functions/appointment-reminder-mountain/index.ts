...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('Starting Mountain timezone appointment reminder function');
    
    // Calculate 24-48 hour window for America/Denver timezone
    const now = new Date();
    const mountainNow = new Date(now.toLocaleString("en-US", {timeZone: "America/Denver"}));
    
    const start24Hours = new Date(mountainNow);
    start24Hours.setDate(start24Hours.getDate() + 1);
    start24Hours.setHours(0, 0, 0, 0);
    
    const end48Hours = new Date(mountainNow);
    end48Hours.setDate(end48Hours.getDate() + 2);
    end48Hours.setHours(23, 59, 59, 999);

    console.log(`Searching for appointments between ${start24Hours.toISOString()} and ${end48Hours.toISOString()} for Mountain timezone`);

    // Query appointments for Mountain timezone clients in the next 24-48 hours
    const { data: appointments, error } = await supabase
      ...
      .select(`
        ...
        status
      `)
      .eq('client_timezone', 'America/Denver')
    ...
    console.log(`Found ${appointments?.length || 0} appointments for Mountain timezone`);

    if (!appointments || appointments.length === 0) {
      return new Response(JSON.stringify({ message: 'No appointments found for Mountain timezone' }), {
        status: 200,
    ...
    return new Response(JSON.stringify({
      message: `Processed ${appointments.length} appointments for ${clientGroups.size} clients in Mountain timezone`,
    ...
    console.error('Error in Mountain timezone reminder function:', error);
