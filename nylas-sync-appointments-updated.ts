...
serve(async (req) => {
  ...
  try {
    ...
    if (isTestMode) {
      console.log('[nylas-sync-appointments] Running in test mode, bypassing authentication');
    ...
    // Make sure to use the 'user' variable instead of directly referencing 'user.id'
